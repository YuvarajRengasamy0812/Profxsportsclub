...
@section('content')
    ...
    <div class="bg-slate-50 p-8">
        <div class="max-w-7xl mx-auto space-y-12">
            ...
            @endif
            @if($errors->any())
                <div class="bg-red-100 text-red-700 px-6 py-4 rounded-2xl font-bold">
                    {{ $errors->first() }}
                </div>
            @endif
                ...
                @foreach($teams as $team)
                    <div
                        ...
                        <div class="flex justify-between mb-8">
                            <div class="p-4 bg-orange-50 text-[#e85a3c] rounded-2xl">
                        ...
                        <div class="flex justify-between items-center mb-6">
                            <div>
                                <div class="text-[10px] font-black text-slate-300 uppercase">Entry Fee</div>
                                <div class="text-lg font-black text-[#e85a3c]">${{ number_format((float)($team->entryFee ?? 0), 2) }}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-[10px] font-black text-slate-300 uppercase">Slots</div>
                                <div class="text-lg font-black text-[#0f172a]">{{ max((int)($team->max_players ?? 0) - (int)($team->booked_players ?? 0), 0) }} / {{ (int)($team->max_players ?? 0) }}</div>
                            </div>
                        </div>

                        <h3 class="text-3xl font-black text-[#0f172a] mb-6">
                            {{ $team->name }}
                        </h3>
    ...
    <div id="createTeamModal" class="fixed inset-0 hidden z-[100] flex items-center justify-center p-6">
    ...
    <div class="relative bg-white w-full max-w-2xl rounded-[1rem] p-10 shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <h3 class="text-3xl font-black mb-8 text-[#0f172a]">Create New Team</h3>

       <form action="{{ route('crmtemssave') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
    @csrf

    <div>
        <label class="block font-bold mb-2">Team Name</label>
        <input type="text" name="team_name" class="w-full p-4 border rounded-xl" required>
    </div>
    ...
    <div>
        ...
        <input type="file" name="logo" class="block w-full text-sm text-gray-500
            ...
            hover:file:bg-blue-100 border" required>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            <label class="block font-bold mb-2">Entry Fee (USD)</label>
            <input type="number" step="0.01" min="0" name="entryFee" class="w-full p-4 border rounded-xl" required>
        </div>
        <div>
            <label class="block font-bold mb-2">Max Players</label>
            <input type="number" min="1" name="max_players" class="w-full p-4 border rounded-xl" required>
        </div>
    </div>

    <div>
        <label class="block font-bold mb-3">Sport Category</label>
        <div id="categoryTabs" class="flex gap-3">
    ...
    <!-- players input -->
    <div>
        <label class="block font-bold mb-3">Players</label>
        <div id="playersContainer" class="space-y-3">
            <div class="flex gap-3">
                <input type="text" name="players[]" class="flex-1 p-3 border rounded-xl" required>
                <button type="button" onclick="removePlayer(this)" class="px-4 rounded-xl bg-red-500 text-white font-black">✕</button>
            </div>
        </div>
        <button type="button" onclick="addPlayer()" class="mt-4 px-4 py-2 bg-[#0f172a] text-white rounded-xl font-bold">+ Add Player</button>
    </div>

    <button type="submit" class="w-full bg-[#e85a3c] text-white py-4 rounded-2xl font-black uppercase tracking-widest">Create Team</button>
</form>
