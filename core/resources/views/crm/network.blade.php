...
<div class="bg-slate-50 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-16">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <!-- Tabs -->
            <div
                class="flex gap-2 p-2 bg-white justify-center items-center rounded-3xl shadow-sm border border-slate-100">
                <button
                    class="tab-btn px-4 sm:px-6 py-2 bg-[#e85a3c] text-white font-bold rounded-xl text-sm sm:text-base"
                ...
                <button
                    class="tab-btn px-4 sm:px-6 py-2 bg-gray-200 text-gray-700 font-bold rounded-xl text-sm sm:text-base"
                    onclick="filterSports('Indoor', this)">Indoor</button>
            </div>
        </div>
...
<div id="modal" class="fixed inset-0 hidden z-50 flex items-center justify-center p-4 sm:p-6">
    ...
    <div
        class="payment-modal relative bg-white w-full max-w-4xl rounded-3xl p-8 sm:p-10 shadow-2xl transform scale-90 opacity-0 transition-all duration-300 modal-animate">
        <button onclick="closeModal()"
            class="absolute top-4 sm:top-6 right-4 sm:right-6 text-gray-500 hover:text-gray-800">
        ...
        <div class="flex gap-2 mb-4 flex-wrap">
            ...
            <button class="tabBtn px-4 text-white py-2 rounded-full font-semibold transition hover:bg-[#0f172a] bg-[#e85a3c]" data-tab="usds">USDT</button>
            <button class="tabBtn px-4 text-white py-2 rounded-full font-semibold transition hover:bg-[#0f172a] bg-[#e85a3c]" data-tab="bank">Bank</button>
        </div>
...
<form id="travelPaymentForm" method="POST" action="{{ route('paymentstore') }}" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="payment_purpose" value="network">
    ...
    <input type="hidden" name="currency" id="currency"> <!-- for USDT -->
...
function renderSports() {
    ...
    filtered.forEach(t => {
        const availableSlots = t.max_networks - t.booked_networks;
        ...
        <div class="bg-white rounded-[2rem] sm:rounded-[3rem] overflow-hidden border border-slate-100 shadow-lg hover:shadow-2xl transition transform hover:scale-105">
            ...
            <div class="p-6 sm:p-10">
                ...
                <button ${isFull||alreadyBooked?'disabled class="w-full py-3 sm:py-4 rounded-2xl bg-gray-400 text-white font-black uppercase cursor-not-allowed"':'onclick="handleBooking('+t.id+')" class="w-full py-3 sm:py-4 rounded-2xl bg-[#0f172a] text-white font-black uppercase hover:bg-[#e85a3c]"'}>
                    ${alreadyBooked?'Booked':(availableSlots<=0?'Full':'Book Session')}
                </button>
...
function bookFreeEvent(team){
    ...
    formData.append('network_id', team.id);
    formData.append('game', team.game);
    formData.append('_token', '{{ csrf_token() }}');

    fetch('{{ route("crmnetworkbook") }}',{method:'POST', body:formData})
    .then(res=>res.json())
    .then(data=>{
        if(data.success){
            ...
            renderSports();
        } else Swal.fire({icon:'warning', title:'Already Booked', text:data.message});
...
tabButtons.forEach(btn=>{
    btn.addEventListener('click',()=>{
        ...
        tabButtons.forEach(b=>b.classList.remove('bg-[#0f172a]'));
        tabButtons.forEach(b=>b.classList.add('bg-[#e85a3c]'));
...
function submitPayment(type, amountInputId, proofInputId) {
    ...
    document.getElementById('payment_gateway').value = type.toLowerCase();
    document.getElementById('payment_amount').value = amount;
 document.getElementById('network_id').value = team.id;
 document.getElementById('game').value = team.game;
    

    // Assign file to correct input
    if (type.toLowerCase() === 'usdt') {
        const usdtInput = document.getElementById('usdt_deposit_proof_input');
        const dt = new DataTransfer();
        dt.items.add(proofInput.files[0]);
        usdtInput.files = dt.files;
    } else {
        const depositInput = document.getElementById('deposit_proof_input');
        const dt = new DataTransfer();
        dt.items.add(proofInput.files[0]);
        depositInput.files = dt.files;
    }
...
// USDS Submit
  document.getElementById('usdsSubmit').addEventListener('click', () => {
    ...
    document.getElementById('bankSubmit').addEventListener('click', () => {
        submitPayment('bank_deposit', 'bankAmount', 'bankProof');
    });
</script>
