...
class HomeController extends Controller
{
    ...
    $aboutSlidercon ='';
    $title_en = '';
        return view('frontEnd.contact',compact('Topic', 'aboutSlidercon','title_en'));
        
    }
    
   public function profx(Request $request) {
 ...
 try {
    ...
    {
		$pagetitle = "About Us";
		$aboutSlidercon = DB::table('topics')->where('webmaster_id', 18)->limit(10)->offset(3)->get();
		return view("frontEnd.custom.about", compact('pagetitle', 'aboutSlidercon'));
	}
	...
	public function leaderboards(Request $request)
    {
		...
		/*$leaderboard = DB::select("SELECT o.login, u.name, u.email, c.title_en AS country, c.code AS ccode, SUM(o.profit) AS total_profit, LEAST(GREATEST(SUM(o.profit), 0), 100) AS profit_percentage FROM smartend_tournaments_deals o LEFT JOIN smartend_tournament_liveaccount li ON li.trade_id = o.login INNER JOIN smartend_users u ON u.email = li.email LEFT JOIN smartend_countries c ON c.id = u.nationalities WHERE o.tournament_id = 1 GROUP BY o.login, u.name, u.email, c.title_en, c.code ORDER BY total_profit DESC");
		$leaderboard = collect($leaderboard);*/
		
		/*Manual*/
		/*$leaderboard = DB::table('leaderboards as lb')
     ...
     $latestleague = DB::table('leagues')
                ...
                ->first();
        //  echo'<pre>';print_r($latestleague);exit;
        ...
        return view("frontEnd.custom.leaderboards", compact('pagetitle', 'leaguecatlist', 'liveaccountData', 'showLeague', 'accounttypeval','latestleague'));
	}
	
	public function leagues($pageslug="")
    {
		$pagetitle = "Leagues";
		$childCategories = '';
		$category = Categorie::where('slug', $pageslug)->first();
		if($category) {
			$childCategories = Categorie::where('parent_id', $category->id)
                                ->where('status', 1)
								->orderBy('id', 'desc')
                                ->get();
		}
		return view("frontEnd.custom.league", compact('pagetitle', 'category', 'childCategories'));
		
		/*$leaguecat = DB::table('sections')->where('seo_url_slug_en', $pageslug)->first();
		$leaguecatlist = DB::table('topic_categories')->where('section_id', $leaguecat->id)->get();	
		$topicIdsval = $leaguecatlist->pluck('topic_id')->toArray();
		$leaguelist = Topic::with('fields')->where('webmaster_id', 20)->whereIn('id', $topicIdsval)->where('status', 1)->orderBy('id','DESC')->get();
		return view("frontEnd.custom.league", compact('pagetitle', 'pageslug', 'leaguelist'));*/
	}
	
	public function leagueslist($pageslug="")
    {
		$pagetitle = '';
		$category = Categorie::where('slug', $pageslug)->first();		
		if($category) {
			$leaguelist = League::where('subcategoryid', $category->id)
                                ->where('status', 1)
								->orderBy('id', 'asc')
                                ->get();
		}
		return view("frontEnd.custom.leaguelist", compact('pagetitle', 'category', 'leaguelist'));
	}
	
	public function leaguesdetail(Request $request)
    {
		$detailslug = $request->query('leagueslug');
		$resluague = League::where('id', $detailslug)->where('status', 1)->first();
		$pagetitle = $resluague->leagurTitle;	
		return view("frontEnd.custom.league-detaiils", compact('pagetitle', 'resluague'));
	}
	
	public function partnership()
    {
		$pagetitle = "Partnership";
		$partnerspecialacc = DB::table('topics')->where('webmaster_id', 22)->get();
		return view("frontEnd.custom.partnership", compact('pagetitle', 'partnerspecialacc'));
	}

	public function blogs()
    {
		$pagetitle = "Latest News";
		$bloglist = Helper::Topics(7, 0, 0);
		return view("frontEnd.custom.blog", compact('pagetitle', 'bloglist'));
	}
	
	
	 public function privacy(){
        return view('frontEnd.custom.privacy_policy');
    }
    public function terms(){
        return view('frontEnd.custom.terms_and_conditions');
    }
     public function referral(){
         $pagetitle = "Referral Program";
		$partnerspecialacc = DB::table('topics')->where('webmaster_id', 200)->get();
        return view('frontEnd.custom.referralpro');
    }
