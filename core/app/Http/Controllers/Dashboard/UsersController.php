...
class UsersController extends Controller
{
    ...
    public function index()
    {
        ...
        if (@Auth::user()->permissionsGroup->view_status) {
            $Users = User::where('created_by', '=', Auth::user()->id)->orwhere('id', '=', Auth::user()->id)->where('user_type', '!=', 2)->orderby('id', 'asc')->paginate(config('smartend.backend_pagination'));
            $Permissions = Permissions::where('created_by', '=', Auth::user()->id)->orderby('id', 'asc')->get();
        } else {
            $Users = User::orderby('id', 'asc')->where('user_type', '!=', 2)->paginate(config('smartend.backend_pagination'));
